...
const VoucherPopup = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const checkResponse = await fetch(`https://app.saincube.com/app1/voucher/check?code=${encodeURIComponent(voucherCode)}`, {
        method: 'POST',
        headers: {
      ...
      const markResponse = await fetch(`https://app.saincube.com/app1/voucher/mark-used?code=${encodeURIComponent(voucherCode)}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}`,
        },
