...
// Dynamic Map Component with SSR disabled
...
const statusConfig = {
  ...
  inBearbeitung: {
    ...
    text: 'Ausstehend: 3 min.',
    border: 'border-yellow-500',
  }
};

// Then update the B-Plan Unit Card section in renderResultView
const getBPlanStatus = () => {
  if (!analysisData?.compliance_status) return statusConfig.inBearbeitung;
  return statusConfig[analysisData.compliance_status] || statusConfig.inBearbeitung;
};
  ...
  useEffect(() => {
    if (analysisData?.analysis_result?.result_data) {
      ...
      const resultFields = {
        ...
        'Projekttyp': analysisData.analysis_result.result_data[' Project type'],
        'Gebäudeklasse': analysisData.analysis_result.result_data[' Building class'],
        // 'Prüfdatum': analysisData.analysis_result.
      };
  ...
  const handleProcess = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/upload-B-Plan/', {
        method: 'POST',
        headers: {
      ...
      // Instead of reloading, handle the response and update the view
      const data = analysisData;
      if (data?.analysis_result?.result_data) {
        // Update the project data with the response
        const mappedFields = {
          'Projekttitel': data.analysis_result.result_data['Project title'],
          'Projekttyp': data.analysis_result.result_data[' Project type'],
          'Gebäudeklasse': data.analysis_result.result_data[' Building class'],
          'Nutzung des Gebäudes': data.analysis_result.result_data[' Building usage'],
          'Anzahl der Stockwerke': data.analysis_result.result_data[' Number of floors']
        };
        setProjectData(mappedFields);

        const resultFields = {
          'Standort': data.analysis_result.result_data[' Project location'],
          'Projekttyp': data.analysis_result.result_data[' Project type'],
          'Gebäudeklasse': data.analysis_result.result_data[' Building class'],
        };
        setResultProjectData(resultFields);
  ...
  const renderNonComplianceDetails = () => {
    const parseNonComplianceDetails = (details) => {
      ...
      // Split the text into sections by comma or period
      const sections = details.split(/[.,]/)
        .map(s => s.trim())
        .filter(Boolean); // Remove empty strings

      // Group the sections into an array of objects with type and content
      const formattedContent = sections.map(section => {
        if (section.endsWith(':')) {
          return {
            type: 'heading',
            content: section.slice(0, -1).trim() // Remove the colon
          };
        }
        return {
          type: 'bullet',
        ...
        <div className="col-span-12 lg:col-span-7">
          <div className="space-y-3">
            <div className='flex flex-col sm:flex-row items-center justify-center gap-x-2'>
              ...
              <div>
                {/* B-Plan Unit Card - Small Version */}
  ...
  return (
    <div className="fixed inset-0 text-black bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="relative bg-white rounded-lg w-full max-w-6xl h-[90vh] flex flex-col">
        <button onClick={onClose} className="absolute top-[-15px] right-[-15px] bg-gray-800 hover:bg-gray-700 text-white rounded-full p-2 z-50">
          <X className="w-5 h-5" />
        </button>
