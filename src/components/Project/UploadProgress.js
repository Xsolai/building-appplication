...
const UploadProgress = ({ isUploading, file, showSelection, onRemove }) => {
  ...
  if (showSelection && file && !isUploading) {
    return (
      <div className="w-full max-w-md mx-auto p-4 bg-blue-50 rounded-xl border border-blue-100">
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white rounded-xl shadow-sm border border-gray-100">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-medium text-gray-900">{file?.name || 'Uploading file...'}</h3>
      ...
      {!isProcessing ? (
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">Uploading your file...</span>
          </div>
        </>
      ) : (
